variable "prefix" {
  description = "Префікс для найменування ресурсів"
  type        = string
}
...
variable "web_port" {
  description = "TCP Порт для доступу до вебсервера"
  type        = number
  validation {
    condition     = var.web_port >= 1024 && var.web_port <= 65535
    error_message = "Помилка: Порт повинен знаходитись у діапазоні (1024-65535)."
  }
}
